...
def printloading(tokenizer_path):
    print(f"Loading {tokenizer_path}")

def getTokenizer(tokenizer_type = "word_60K"):
    if tokenizer_type == "word_60K":
        tokenizer_dir = "tokenizers/word_60k_tokenizer"
        tokenizer_path = tokenizer_dir + "/tokenizer.pth"
        printloading(tokenizer_path)
    ...
    if tokenizer_type == "word_30K":
        tokenizer_dir = "tokenizers/word_30k_tokenizer"
        tokenizer_path = tokenizer_dir + "/tokenizer.pth"
        printloading(tokenizer_path)
        vocab_path = tokenizer_dir + "/vocab.pkl"
        loaded_tokenizer = torch.load(tokenizer_path)
    ...
    ## run the following code if the tokenizer is other than word tokenizer
    tokenizer_dir = "tokenizers"
            
    if tokenizer_type == 'word_piece':
        tokenizer_dir += "/word_piece_tokenizer"
        tokenizer_path = tokenizer_dir + "/tokenizer_wp.pickle"
        printloading(tokenizer_path)
        
    
    elif tokenizer_type == 'sentence_piece':
        tokenizer_dir += "/sentence_piece_tokenizer"
        ...
        tokenizer_path = tokenizer_dir + "/morpheme_tokenizer.pth"
        printloading(tokenizer_path)
